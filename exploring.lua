...
local function xmltotable(xml)
	local function get(file,replace)
		local base = "https://raw.githubusercontent.com/manoelcampos/xml2lua/master/"
		local str = game:HttpGet(base..file,true)
		if replace then
			str = str:gsub(unpack(replace))
		end
		local f = loadstring(str)
		setfenv(f,setmetatable({require = function(file) return get(file..".lua") end},{__index = getfenv(1)}))
		return f()
	end
	local main,handler = get("xml2lua.lua"),get("xmlhandler/tree.lua")--,{"node._attr=tag.attrs","if tag and tag.attrs and type(tag.attrs)=='table' then for a,b in pairs(tag.attrs) do node[a]=b end else node._attr=tag.attrs end"})
	main.parser(handler):parse(xml)
	return handler.root
end

local ExplorerIndex, ReflectionMetadata = {}, game:HttpGet(speedload and 'https://raw.githubusercontent.com/KnuxTheFixer/dex/main/xmlJSON.lua' or 'https://raw.githubusercontent.com/CloneTrooper1019/Roblox-Client-Tracker/roblox/ReflectionMetadata.xml')
if speedload then
  ReflectionMetadata = HttpService:JSONDecode(ReflectionMetadata)
else
  ReflectionMetadata = xmltotable(ReflectionMetadata)
end

for _, Metadata in ipairs(ReflectionMetadata.roblox.Item[1].Item) do
	local Item = Metadata.Properties.string
	local ImageOrder = 0
	local ClassName = "Instance"
	for Category, Data in ipairs(Item) do
		if Data._attr and Data._attr.name == "ExplorerImageIndex" then
			ImageOrder = tonumber(Data[1])
		end
		if Data._attr and Data._attr.name == "Name" then
			ClassName = Data[1]
		end
	end
...
if RunningScriptsStorageEnabled then
	...
	if not GetSetting_Bindable:Invoke("RSSIncludeRL") then
		for _, RunningScript in next, getscripts() do
			...
			pcall(function()
				if not IsA(runningScript,"ModuleScript") then
		...
		for _, Instance in next, getinstancelist() do
			if typeof(Instance) == "Instance" and IsA(Instance, "LuaSourceContainer") then
				...
				pcall(function()
					if not IsA(Script,"ModuleScript") then
...
if NilStorageEnabled then
	...
	for _, NilInstance in next, getnilinstances() do
		...
		pcall(function()
			...
			if IsA(NilInstance,"LuaSourceContainer") and not IsA(NilInstance,"ModuleScript") then
